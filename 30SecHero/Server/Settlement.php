...
$update = mysql_query("UPDATE ".$db_name.".playeraccount SET `gold` = '".$gold."', `emerald` = '".$emerald."', `maxFloor` = '".$maxFloor."' WHERE `id` = '".$id."'",$con_w);
if($update)
{
	if($equipStr=="")
	{
		...
		die("Success:".$gold."/".$emerald."/".$maxFloor.": \nExecuteTime=".$executeTime);
	}
	else
	{
		...
		{			
			...
			$time_end = microtime(true);
			$executeTime = $time_end - $time_start;
			die("Success:".$gold."/".$emerald."/".$maxFloor."/".$uidStr.": \nExecuteTime=".$executeTime);
		}
		else
		{
